  ...
  return (
      <MenubarPortal>
        <MenubarPrimitive.Content
            ...
            className={cn(
                // z-[1000], not z-50: in continuous view each page div gets an
                // inline `zIndex` up to `pages.length` (see `ContinuousWorkspace`)
                // so overflowing content paints in reading order. With enough
                // pages that easily exceeds Tailwind's z-50, so this portaled
                // dropdown (which is `position: fixed` at the document root, not
                // nested under the page) would render *behind* the page content.
                // 1000 is the same ceiling `TextBlockLayer`'s guides/badge use.
                'z-[1000] min-w-[12rem] origin-(--radix-menubar-content-transform-origin) overflow-hidden rounded-md border bg-popover p-1 text-popover-foreground shadow-md data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95',
                className,
            )}
            {...props}
        />
...
function MenubarSubContent({
  ...
  return (
      <MenubarPrimitive.SubContent
          ...
          className={cn(
              // Same reasoning as `MenubarContent` above — submenus need to clear
              // the highest `zIndex` any continuous-view page can reach too.
              'z-[1000] min-w-[8rem] origin-(--radix-menubar-content-transform-origin) overflow-hidden rounded-md border bg-popover p-1 text-popover-foreground shadow-lg data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95',
              className,
          )}
          {...props}
      />
