...
import { FontReplacePanel } from '@/components/panels/FontReplacePanel'
...
export function Panels() {
  ...
  return (
      <div className='flex h-full min-h-0 w-full flex-col border-l bg-muted/50'>
        ...
        >
          <TabsList className='m-2 mb-0 grid w-[calc(100%-1rem)] grid-cols-3 bg-muted/70'>
            ...
            <TabsTrigger value='fontReplace' data-testid='panels-tab-font-replace' className='gap-1'>
              <RepeatIcon className='size-3.5' />
              <span className='text-xs font-semibold tracking-wide uppercase'>
              {t('panels.fontReplace')}
            </span>
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent
              value='fontReplace'
              className='min-h-0 flex-1 px-1 pb-2 data-[state=inactive]:hidden'
              data-testid='panels-font-replace'
          >
            <ScrollArea className='h-full' viewportClassName='pr-1 [&>div]:!block'>
              <FontReplacePanel />
            </ScrollArea>
