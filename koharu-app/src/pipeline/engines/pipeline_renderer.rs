...
impl Engine for Model {
    async fn run(&self, ctx: EngineCtx<'_>) -> Result<Vec<Op>> {
        // Temporary diagnostic span: if a block is rejected before raster
        // allocation, the renderer's warning retains the page identity.
        let page_name = ctx
            .scene
            .page(ctx.page)
            .map(|page| page.name.as_str())
            .unwrap_or("<missing>");
        let _diagnostic_span = tracing::info_span!(
            "renderer_diagnostic_page",
            page_id = %ctx.page,
            page_name = %page_name,
        )
        .entered();
